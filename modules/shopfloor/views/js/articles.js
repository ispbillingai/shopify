 ...
 * Warehouse articles.
 *
 * Same working rhythm as the Magazzino screen (scan, pick, say how many, save),
 * against the gestionale article table rather than shop stock. Quantities are
 * decimal here, because the export carries goods sold by weight and by length.
 */

(function () {
    'use strict';

    var root = document.getElementById('shopfloor-articles');
    ...
    // Trailing zeros on a whole number read as noise on a shelf.
    function qty(value) {
    ...
    function renderResults(rows) {
        ...
        rows.forEach(function (row) {
            ...
            name.textContent = row.description || row.code;
            left.appendChild(name);

            // A billing line, not stock. Marked so nobody loads goods against it.
            if (row.is_service) {
                var badge = document.createElement('span');
                badge.className = 'shopfloor__badge';
                badge.textContent = t('service_line', 'service');
                left.appendChild(badge);
            }

            var meta = document.createElement('span');
            meta.className = 'shopfloor__result-meta';
            meta.textContent = [row.code, row.location, row.supplier].filter(Boolean).join(' · ');
            ...
            stock.className = 'shopfloor__result-stock' + (Number(row.quantity) <= 0 ? ' is-empty' : '');
            stock.textContent = qty(row.quantity) + ' ' + t('in_stock', 'in stock');
    ...
    function select(row) {
        ...
        document.getElementById('shopfloor-selected-name').textContent = row.description || row.code;
        document.getElementById('shopfloor-selected-meta').textContent =
            [row.code, row.barcode].filter(Boolean).join(' · ');
        document.getElementById('shopfloor-selected-where').textContent =
            [row.location, row.class, row.supplier].filter(Boolean).join(' · ');
        document.getElementById('shopfloor-selected-stock').textContent = qty(row.quantity);
        ...
        quantityInput.value = mode === 'intake' ? '1' : qty(row.quantity);
        quantityInput.focus();
    ...
    function setMode(next) {
        ...
        if (selected) {
            quantityInput.value = next === 'intake' ? '1' : qty(selected.quantity);
        }
    ...
    function apply() {
        ...
        var raw = String(quantityInput.value).replace(',', '.');
        ...
        call(mode === 'intake' ? 'intake' : 'correct', {
            id_article: selected.id_article,
            quantity: raw,
            ...
            .then(function (data) {
                ...
                document.getElementById('shopfloor-selected-stock').textContent = qty(data.quantity_after);

                showDone(
                    (selected.description || selected.code) + ': '
                    + qty(data.quantity_before) + ' → ' + qty(data.quantity_after)
                    + ' (' + (data.delta > 0 ? '+' : '') + qty(data.delta) + ')'
                );

                document.getElementById('shopfloor-art-instock').textContent = data.summary.in_stock;
                document.getElementById('shopfloor-art-value').textContent = data.value_display;

                renderLog(data.movements);

                noteInput.value = '';
                quantityInput.value = mode === 'intake' ? '1' : qty(data.quantity_after);
                searchInput.focus();
                searchInput.select();
            })
            .catch(function (error) {
    ...
    searchInput.addEventListener('keydown', function (event) {
        ...
        // A scan is an exact code, so take an exact match straight away.
        var exact = lastResults.filter(function (row) {
            return String(row.code).toLowerCase() === term
                || String(row.barcode).toLowerCase() === term;
        });
